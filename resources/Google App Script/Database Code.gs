...
function newAnonymousUser(){
  ...
  let signin_response = UrlFetchApp.fetch(variables.database.signup + variables.database.ID, signin_options);
  ...
  let setup_response = UrlFetchApp.fetch(variables.database.url + "?documentId=" + script.getProperty("userID"), setup_options);
...
function refreshDatabaseToken() {
  ...
  let refresh_response = UrlFetchApp.fetch(variables.database.refresh + variables.database.ID, refresh_options);
 ...
 *
 * @param {Integer} timestamp - timestamp to ping the server with
...
function pingDatabase(timestamp, retry = false) {
  ...
  let ping_options = {
    ...
    'headers': {'Authorization': 'Bearer ' + script.getProperty("auth_token") + ''},
    'contentType': 'application/json',
    'payload': "{'name':'projects/phone-grown/databases/(default)/documents/anonymous/" + script.getProperty("userID") +"','fields':{'ping':{'integerValue':'" + timestamp + "'},}}",
    'muteHttpExceptions': true
  };
  let ping_response = UrlFetchApp.fetch(variables.database.url + script.getProperty("userID"), ping_options);
