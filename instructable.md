...
1. Choose the _Add row to spreadsheet_ action and fill in the details. These details need to be very accurate:
    ...
    1. Remove everything in the **_Formatted row_**, and replace it with something more readable, such as _'temperature'_. We will need this name later.
...
Now, we will set up the Space Station applet. Follow the same steps above, but for the **_If This_** look for _Space > ISS passes over a specific location_. Also follow the exact same steps for the **_Then That_**, except give the **Formatted row** a different name, for example 'ISS'.
...
## Step 5: setup the colours
In step 5, we will set which area of the phone will light up a certain colour when either the temperature rises or the ISS passes over. To give these colours more context and to customise your smart display to feel more homely, we need a drawing to put in front of the phone.
TEST!

// rules and whatnot
- coloured areas overlap and mix! E.g. (pictures). Can create up to 10 backgrounds (or reuse them)
